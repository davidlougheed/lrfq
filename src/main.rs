...
fn fastq_report() -> Result<FastqStatsReport, Error> {
    ...
    let mut reader: fastq::io::Reader<BufReader<File>> = File::open("bc1015.fq").map(BufReader::new).map(fastq::io::Reader::new)?;
    ...
    loop {
        ...
        stats_agg.read_lengths.push(seq_len);
        stats_agg.total_bases += seq_len;
        stats_agg.total_gc += seq.iter().filter(|&&b| b == b'G' || b == b'g' || b == b'C' || b == b'c').count();
    ...
    Ok(FastqStatsReport {
        ...
        median_length: median(&stats_agg.read_lengths),
